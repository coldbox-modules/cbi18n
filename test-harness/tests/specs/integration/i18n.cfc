	...
	function run() {
		describe( "i18n module", function() {
			...
			it( "can load from modules", function() {
				...
				expect( event.getValue( "cbox_rendered_content" ) ).toInclude(
					"Welcome to my awesome multi-lingual module"
				);
			} );
			...
			it( "can load a JSON resource in the parent from a flat JSON file", function() {
				var event = execute( event = "main.testi18nJsonResourceService", renderResults = true );
				expect( event.getValue( "cbox_rendered_content" ) ).toInclude( "Flat JSON" );
			} );

			it( "can load a JSON resource in the parent from a nested JSON file", function() {
				var event = execute( event = "main.testi18nNestedJsonResourceService", renderResults = true );
				expect( event.getValue( "cbox_rendered_content" ) ).toInclude( "Nested JSON" );
			} );
