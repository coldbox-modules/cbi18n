...
component singleton accessors="true" {
    ...
    string function formatRBString(required rbString, required substituteValues) {
        ...
        if (isArray(arguments.substituteValues)) {
            ...
            for (var x = 1; x lte valLen; x = x + 1) {
                tmpStr = tmpStr.replace('{#x#}', arguments.substituteValues[x], 'ALL');
            }
        ...
        // Struct substitutions by key
            ...
            for (var thisKey in arguments.substituteValues) {
                tmpStr = tmpStr.replaceNoCase('{#lCase(thisKey)#}', arguments.substituteValues[lCase(thisKey)], 'ALL');
            }
            return tmpStr;
        }

        // Single simple substitution
        return arguments.rbString.replace('{1}', arguments.substituteValues, 'ALL');
    }
     ...
     */
    string function messageFormat(required string thisPattern, required args, thisLocale = '') {
